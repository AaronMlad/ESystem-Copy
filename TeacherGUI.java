...
public class TeacherGUI extends javax.swing.JFrame {
    ...
    private void SaveActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        teach.SaveRecord(teachName.getText(), teachAdd.getText(), 
                        teachContact.getText(), teachDept.getText());
        
        // Get the last inserted teacher ID
        ResultSet rs = ESystem.st.executeQuery("SELECT LAST_INSERT_ID()");
        if (rs.next()) {
            int teacherId = rs.getInt(1);
            String name = teachName.getText().trim();
            ...
            // Get first letter of the name, lowercase
            String firstLetter = name.substring(0, 1).toLowerCase();
            firstLetter = firstLetter.replaceAll("[^a-z]", "t"); // default to 't' if not a letter
            
            // Create username as ID + full name without spaces (lowercase)
            String username = teacherId + name.replaceAll("\\s+", "").toLowerCase();
            ...
            if (userCheck.next()) {
                // User exists, just grant privileges on the current database
                ...
                JOptionPane.showMessageDialog(this, 
                    ...
                    "Existing database user granted access to this semester:\n" +
                    "Username: " + username);
            } else {
                // User doesn't exist, create new user
                String password = "AdDU" + name.replaceAll("\\s+", "");
                ...
                JOptionPane.showMessageDialog(this, 
                    "Teacher saved successfully!\n" +
                    "New database user created with SELECT/INSERT/UPDATE privileges:\n" +
                    "Username: " + username + "\n" +
                    "Password: " + password);
            }
        }
        ...
        showRecords();
            ...
            System.err.println("Error in SaveActionPerformed:");
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this,  
                "Error: " + ex.getMessage(),
    ...
    private void DeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_DeleteActionPerformed
        try {
            int teacherId = Integer.valueOf(teachId.getText());
            String teacherName = teachName.getText().trim();

            if (teacherName.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Cannot delete: Teacher name is empty",
                    "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }

            // Create username as ID + full name without spaces (lowercase)
            String username = teacherId + teacherName.replaceAll("\\s+", "").toLowerCase();

            // Revoke all privileges on the current database for this user
            ...
            // Delete the teacher record from the current database
            Teachers teach = new Teachers();
            teach.DeleteRecord(teacherId);

            JOptionPane.showMessageDialog(this,
                "Teacher deleted successfully!\n" +
                "Database user permissions revoked for this semester:\n" +
                "Username: " + username + "\n" +
                "Database: " + ESystem.db);
            ...
            System.err.println("Error in DeleteActionPerformed:");
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this,
